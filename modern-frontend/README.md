...
Figma file), integrated into the FixKart monorepo as the **main (and only) site**.
...
The React app is a full storefront:
...
- **Cart → Checkout → Orders** — localStorage cart, address capture, order
  creation, and an order confirmation + order history.
...
- **Backend:** the Vite dev server proxies `/api` to the FixKart Express
  ...
  uses relative URLs with no CORS setup. In Docker the nginx container does the
  same proxying.
- **Auth:** sessions are stored under `fixkart_session` in localStorage.
- **Cart:** stored under `fixkart_cart` in localStorage.
